...
| 决策 | 结论 |
|---|---|
| 开发者准入 | Discourse 信任等级 TL1 及以上，账号正常且未被禁言 |
| 应用创建 | 用户自助创建并提交人工审核 |
...
论坛用户天然是 Connect 用户；只有达到 TL1 且账号正常、未被禁言的用户才获得创建和提交应用的开发者资格。
...
审核员不能审核自己拥有的应用。批准、驳回、暂停、恢复和管理员越权操作必须填写理由并记录审计。
