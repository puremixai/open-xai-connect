...
3. 提交审核。普通用户需要 TL1 及以上、活跃且未被禁言；`connect-admins` 群组成员在账号活跃、未被禁言且未暂停时可绕过 TL1 创建/提交。每个用户最多三个开放中的应用。
4. 审核通过后应用直接进入生产状态，页面显示 Client ID。Client Secret 默认掩码；所有者完成近期敏感操作确认后可以查看或轮换。
